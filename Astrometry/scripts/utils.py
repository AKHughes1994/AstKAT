...
def BootstrapPositions(data, n_bootstrap=1000):
    ...
    ################
    # Aug 8th 2023 # -- Need to figure out how to ensure we don't only sample NaN values
    ################
    ...
    mask = np.isnan(bootstrap_data) # replace Nan values with near neighboor (Probably need to change this)
    bootstrap_data[mask]   = np.interp(np.flatnonzero(mask), np.flatnonzero(~mask), bootstrap_data[~mask])
